...
namespace School.Data
{
    public class SqlRepository : IRepository
    {
        ...
        public async Task EnterNewStudentAsync(Student student)
        {
            int zipint = 0;
            Int32.TryParse(student.zip, out zipint);
            using SqlConnection connection = new(_connectionString);
            await connection.OpenAsync();

            string cmdText = @"INSERT INTO [School].[Students] (Id, Name, Email, Address1, Address2, City, State, Zip, GPA) VALUES (@id, @name, @email, @a1, @a2, @city, @state, @zip, @gpa);";
            ...
            cmd.Parameters.AddWithValue("@a1", student.address1);
            cmd.Parameters.AddWithValue("@a2", student.address2);
            cmd.Parameters.AddWithValue("@city", student.city);
            cmd.Parameters.AddWithValue("@state", student.state);
            cmd.Parameters.AddWithValue("@zip", zipint);
            cmd.Parameters.AddWithValue("@gpa", student.gpa);

            await cmd.ExecuteNonQueryAsync();
            await connection.CloseAsync();

            _logger.LogInformation("Executed EnterNewStudentAsync, student id #{0}: {1} - {2} created.", student.studentId, student.name, student.email);
        }
        ...
        public async Task DeleteStudentByIdAsync(int id)
        {
            using SqlConnection connection = new SqlConnection(this._connectionString);
            await connection.OpenAsync();
            string cmdText = "DELETE FROM [School].[Students] WHERE Id = @id;";
            using SqlCommand cmd = new SqlCommand(cmdText, connection);
            cmd.Parameters.AddWithValue("@id", id);
            ...
            _logger.LogInformation("Executed DeleteStudentByIdAsync, student id #{0} deleted", id);
        }
